...
public partial class ColorPalatte : ContentView
{
    ...
    internal event EventHandler<Microsoft.Maui.Graphics.Color>? StickyStrokeColorChanged;
    internal event EventHandler<Microsoft.Maui.Graphics.Color>? StickyFillColorChanged;
...
#endif
            StickyFillColorChanged?.Invoke(this, GetColor(button));
        }
        if (selectedColorButtonHighlight.Parent == null)
        {
    ...
    private void ColorStrokeButton_Clicked(object sender, EventArgs e)
    {
        ...
        if (sender is Button button)
        {
            ...
            StickyStrokeColorChanged?.Invoke(this, GetColor(button));
        }
        if (selectedColorButtonHighlightStroke.Parent == null)
        {
    ...
    private void StickyStrokeOpacitySlidervalue_Chnaged(object sender, EventArgs e)
    {
        float opacity = (float)stickyStrokeOpacitySlider.Value;
        SelectedOpacity = opacity;
        StrokeOpacityChanged?.Invoke(this,opacity);
    }

    private void StickyFillColorOpacitySlidervalue_Chnaged(object sender, EventArgs e)
    {
        float opacity = (float)stickyFillColorOpacitySlider.Value;
